...
for id, info in infos.items():
    ...
    for level in range(len(info["difficulty"])):
        with ZipFile("phira/%s/%s-%s.pez" % (levels[level], id, levels[level]), "x") as pez:
            try:
                print("Chart_%s/%s.json" % (levels[level], id), "%s.json" % id, "IllustrationLowRes/%s.png" % id,
                      "%s.png" % id, "music/%s.ogg" % id, "%s.ogg" % id)
                pez.writestr("info.txt",
                             "#\nName: %s\nSong: %s.ogg\nPicture: %s.png\nChart: %s.json\nLevel: %s Lv.%s\nComposer: %s\nIllustrator: %s\nCharter: %s" % (
                             info["Name"], id, id, id, levels[level], info["difficulty"][level], info["Composer"],
                             info["Illustrator"], info["Chater"][level]))
